...
import addConsultancyLogo from "../../../../database/migrations/004_add_consultancy_logo.sql";
...
const MIGRATIONS: { name: string; sql: string }[] = [
  ...
  { name: "004_add_consultancy_logo.sql", sql: addConsultancyLogo },
];
...
/**
 * Executes a single SQL statement (trims and ignores empty).
 * NOTE: Avoid sending multiple DDL statements in one call.
 */
async function execSql(client: PoolClient, sql: string) {
  const stmt = sql.trim();
  if (!stmt) return;
  // Ensure it ends with ; (optional)
...
/**
 * Minimal SQL splitter for DDL files:
 * - Splits on semicolons.
 * - Strips empty parts.
 *
 * NOTE: This is not suitable for SQL function bodies containing semicolons.
 * Keep migration files to simple DDL/DDL+simple DML.
 */
